...
//    var jq = $.noConflict();
 $(function(){
  ...
  $("#list").sortable({class:'move',update:function(){
    ...
    // alert(order);
    $.post(strURL+"appadmins/modelphotoOrder",order,function(theResponse){
    });
   }
  });
 });
                        ...
                        <tbody id="list">
                            <?php foreach ($dataListings as $list): ?>
                                <tr id="arrayorder_<?php echo $list->id; ?>" class="message_box ui-sortable-handle">
                                    ...
                                    <td style="text-align:  left;"><img width="50" src="<?php echo HTTP_ROOT.PHOTOS.@$id.'/'.$list->gal_name.'/'.$list->files_name;?>" >
                                    </td> 
                                    <td style="text-align: center; width: 20%">
<!--                                        <a href="<?php echo HTTP_ROOT . 'appadmins/mainPhoto/' . $list->id . '/ModelPhotos'; ?>">
                                        ...
                                        <?php if ($list->is_active == 1) { ?>
                                            <a href="<?php echo HTTP_ROOT . 'appadmins/deactive/' . $list->id . '/ModelPhotos'; ?>"><?= $this->Form->button('<i class="fa fa-check"></i>', ["data-placement" => "top", "data-hint" => "Active", 'class' => "btn btn-success hint--top  hint", 'style' => 'padding: 0 7px!important;']) ?> </a>
                                        <?php } else { ?>
                                            <a href="<?php echo HTTP_ROOT . 'appadmins/active/' . $list->id . '/ModelPhotos'; ?>"><?= $this->Form->button('<i class="fa fa-times"></i>', ["data-placement" => "top", "data-hint" => "Inactive", 'class' => "btn btn-danger hint--top  hint", 'style' => 'padding: 0 7px!important;']) ?></a>
                                        <?php } ?>
                                        <?= $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-trash')), ['action' => 'delete', $list->id, 'ModelPhotos'], ['escape' => false, "data-placement" => "top", "data-hint" => "Delete", 'class' => 'btn btn-danger hint--top  hint', 'style' => 'padding: 0 7px!important;', 'confirm' => __('Are you sure you want to delete  ?')]); ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    ...
                    <a class="cxa btn btn-warning" href="<?=HTTP_ROOT.'appadmins/create_photo';?>" class="next"> &laquo; Back</a>
