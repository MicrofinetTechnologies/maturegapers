...
<div class="content-wrapper">
    <section class="content-header">
        <h1> Scenes Photo Gallery </h1>
        <ol class="breadcrumb">
            <li><a href="<?= h(HTTP_ROOT) ?>appadmins"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active" ><a href="<?= h(HTTP_ROOT) ?>appadmins/scenesphoto_gallery"><i class="fa fa-list"></i> Scenes Photo Gallery</a></li>
        </ol>
    </section>
            ...
            <div class="col-xs-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        <li id='li_photo' class="active"><a href="<?php echo HTTP_ROOT . 'appadmins/scenesphoto_gallery/' . $id ?>">Scenes photo</a></li>
                    </ul>
                    
                    <div class="tab-content">
                        <div class="tab-pane active" id="photo"> 
                            <?=$this->Form->create(null,['class'=>"form-horizontal",'type'=>'POST','data-toggle'=>"validator", 'novalidate'=>"true", 'enctype'=>'multipart/form-data', 'id'=>'form1']);?>
                            
                           
                               
                                <div class="form-group">
                                    <label for="inputEmail" class="col-sm-2 control-label">Scene Name <span style="color:#FF0000">*</span></label>
                                    <div class="col-sm-6">
                                        <?= $this->Form->select('scenes_id', $options, ['class' => "form-control", 'label' => false, 'onchange' => 'setUrl(this.value)', 'default' => @$id,'id'=>'scenes_id']); ?>
                            ...
                            <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <?php echo $this->Form->input('photo_filess.', array('type' => 'file', 'multiple'));?>
<!--                                        <input type="file" name='photo_filess' id='photo_filess' multiple>-->
                                ...
                                <div class="form-group">
                                    <div class="col-sm-offset-2 col-sm-10">
                                        <button type="submit" name='__submit__' value="4" id='photo_save' class="btn btn-danger">Save</button>  
                        ...
                        <tbody id="list">
                            <?php foreach ($photo_gallery as $list): ?>
                                <tr id="arrayorder_<?php echo $list->id; ?>" class="message_box ui-sortable-handle">
                                    ...
                                    <td>
                                        <a href="<?php echo HTTP_ROOT . 'appadmins/managemscenes_photos/'.$id.'/' . $list->name ; ?>"><?= $this->Form->button('<i class="fa fa-picture-o"></i>', ["data-placement" => "top", "data-hint" => "Manage Photos", 'class' => "btn btn-info hint--top  hint", 'style' => 'padding: 0 7px!important;']) ?> </a>
                                        
                                        <?php if ($list->is_active == 1) { ?>
                                            <a href="<?php echo HTTP_ROOT . 'appadmins/deactive/' . $list->id . '/ScenesPhotoGallery'; ?>"><?= $this->Form->button('<i class="fa fa-check"></i>', ["data-placement" => "top", "data-hint" => "Active", 'class' => "btn btn-success hint--top  hint", 'style' => 'padding: 0 7px!important;']) ?> </a>
                                        <?php } else { ?>
                                            <a href="<?php echo HTTP_ROOT . 'appadmins/active/' . $list->id . '/ScenesPhotoGallery'; ?>"><?= $this->Form->button('<i class="fa fa-times"></i>', ["data-placement" => "top", "data-hint" => "Inactive", 'class' => "btn btn-danger hint--top  hint", 'style' => 'padding: 0 7px!important;']) ?></a>
                                        <?php } ?>
                                            
                                         
                                       
                                        <?= $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-trash')), ['action' => 'delete', $list->id, 'ScenesPhotoGallery'], ['escape' => false, "data-placement" => "top", "data-hint" => "Delete", 'class' => 'btn btn-danger hint--top  hint', 'style' => 'padding: 0 7px!important;', 'confirm' => __('Are you sure you want to delete  ?')]); ?>
                                            
                                            <?php if ($list->type == 1) { ?>
                                            <a href="<?php echo HTTP_ROOT . 'appadmins/freeorpaid/' . $list->id . '/ScenesPhotoGallery/2/type'; ?>"><?= $this->Form->button('<i class="fa fa-eye"></i>', ["data-placement" => "top", "data-hint" => "Free can see.", 'class' => "btn btn-warning  hint--top  hint", 'style' => 'padding: 0 7px!important;']) ?> </a>
                                        <?php } else { ?>
                                            <a href="<?php echo HTTP_ROOT . 'appadmins/freeorpaid/' . $list->id . '/ScenesPhotoGallery/1/type'; ?>"><?= $this->Form->button('<i class="fa fa-eye-slash"></i>', ["data-placement" => "top", "data-hint" => "Free can't see.", 'class' => "btn btn-default hint--top  hint", 'style' => 'padding: 0 7px!important;']) ?></a>
                                        <?php } ?>
                                    </td>
...
$(my_form_id).on( "submit", function(event) { 
	...
	$('.progress').show();
        var scenes_id = $("#scenes_id option:selected" ).val();
	...
	if(!window.File && window.FileReader && window.FileList && window.Blob){ //if browser doesn't supports File API
		...
		if(proceed){
			...
			var form_data = new FormData(this); //Creates new FormData object
                        form_data.append('scenes_id', scenes_id);
