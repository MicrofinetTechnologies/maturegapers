...
class UserHelper extends Helper {
    ...
    function topscenesHelper() {
        ...
        if ($view_type->type == "Last created") {
            ...
            $query = $table->find('all')->order(['Scenes.id' => 'DESC'])->where(['is_active' => 1, 'releasedate <='=>date('Y-m-d')])->first();
        }
        ...
        $query1 = $video->find('all')->where(['ScenesVideos.scene_id' => $query->id, 'ScenesVideos.is_tailer' => 'Is tailer'])->order(['sort_order' => 'ASC'])->first();
        ...
        $fulldatalist['video_duration'] = @$video->find('all')->where(['ScenesVideos.scene_id' => $query->id, 'ScenesVideos.is_tailer' => 'Is main'])->order(['sort_order' => 'ASC'])->first()->video_durations;
