...
//    $this->loadComponent('Phpoffice');
        ...
        $this->loadModel('SceneSettings');
        $this->viewBuilder()->layout('admin');
        $this->Site_Nm = $this->GeneralSettings->find('all')->where(['id' => 1])->first()->sitename;
    }

    public function beforeFilter(Event $event) {
        $this->Auth->allow(['videoDelete', 'settingBanner', 'modelUrl', 'modelalldata', 'generalSetting', 'skincolor', 'modelOrder', 'createGallary', 'getFiles', 'updatefolder', 'updateftpvideo']);
    }
    ...
    public function deleteminiaturePic($id = null) {

        $this->Models->updateAll(['miniature_pic' => ''], ['id' => $id]);
        $this->Flash->success(__('Model miniature picture deleted successfully.'));
        return $this->redirect(HTTP_ROOT . 'appadmins/model_mouseover/' . $id);
        exit;
    }
    ...
    public function delete($id = null, $table = null) {
        ...
        if ($table == 'Scenes') {
            ...
            $dir = 'scenevideos/' . $this->Custom->makeSeoUrl($getDetail->scene_name) . '-' . $id . '/' . 'is-trailer';
            $dir1 = 'scenevideos/' . $this->Custom->makeSeoUrl($getDetail->scene_name) . '-' . $id . '/' . 'is-main';
            $dir2 = 'scenevideos/' . $this->Custom->makeSeoUrl($getDetail->scene_name) . '-' . $id . '/' . 'is-bonus';
            $dir3 = 'scenevideos/' . $this->Custom->makeSeoUrl($getDetail->scene_name) . '-' . $id;
            ...
            rmdir($dir3);
            //echo getcwd();
            //echo $this->Custom->makeSeoUrl($getDetail->scene_name).'-'.$id;
            $this->Flash->success(__('Scenes has been deleted.'));
            return $this->redirect(HTTP_ROOT . 'appadmins/scenes_list');
        }
        if ($table == 'Model') {
        ...
        if ($table == 'Dbtable') {
            ...
            return $this->redirect(HTTP_ROOT . 'appadmins/manage_members/');
        }
        if ($table == 'ScenesVideos') {

            if ($getDetail->is_tailer == "Is tailer") {
                echo @unlink(VIDEOS . $getDetail->scene_id . '/4K_' . $getDetail->video_file);
                echo @unlink(VIDEOS . $getDetail->scene_id . '/1080p_' . $getDetail->video_file);
                echo @unlink(VIDEOS . $getDetail->scene_id . '/720p_' . $getDetail->video_file);
                echo @unlink(VIDEOS . $getDetail->scene_id . '/360p_' . $getDetail->video_file);
                echo @unlink(VIDEOS . $getDetail->scene_id . '/' . $getDetail->video_file);
...
//            unlink(PHOTOS . 'scenes/' . $getDetail->scene_id . '/' . $getDetail->gal_name . '/thumb_' . $getDetail->files_name);
    ...
    public function freeorpaid($id = null, $table = null, $value = null, $column = null) {
        $this->$table->updateAll([$column => $value], ['id' => $id]);
        $getDetail = $this->$table->find('all')->where([$table . '.id' => $id])->first();
        if ($table == 'ScenesPhotoGallery') {

            $this->Flash->success(__('Scene Setting updated.'));
            return $this->redirect(HTTP_ROOT . 'appadmins/scenesphoto_gallery/' . $getDetail->scenes_id);
        }
    }
    ...
    public function scenes($id = null) {
        ...
        if ($this->request->is('post')) {
            ...
            if(empty(@$id)){
                ...
                $data['is_active'] = $editData->is_active;
            }
            
            if(!empty($data['id'])){
                if($editData->scene_name != $data['scene_name']){
                 
                    $dir = 'scenevideos/' . $this->Custom->makeSeoUrl($editData->scene_name) . "-" . $editData->id . '/' . 'is-trailer';
                    $dir1 = 'scenevideos/' . $this->Custom->makeSeoUrl($editData->scene_name) . "-" . $editData->id . '/' . 'is-main';
                    $dir2 = 'scenevideos/' . $this->Custom->makeSeoUrl($editData->scene_name) . "-" . $editData->id . '/' . 'is-bonus';
                    $dir3 = 'scenevideos/' . $this->Custom->makeSeoUrl($editData->scene_name) . "-" . $editData->id;
                    array_map('unlink', glob("$dir/*.*"));
                    array_map('unlink', glob("$dir1/*.*"));
                    array_map('unlink', glob("$dir2/*.*"));
                    array_map('unlink', glob("$dir3/*.*"));
            ...
            if ($this->Scenes->save($scenes)) {
                ...
                $url = HTTP_ROOT . 'appadmins/getFiles/';
                $ch = curl_init();
...
//                }
    ...
    public function modelSetting($model_id = null) {
        $dataListings = $this->Models->find('all')->order(['sort_order'=>'ASC']);
        $this->set(compact('dataListings'));
    }

    public function scenesList($model_id = null) {
        $dataListings = $this->Scenes->find('all')->order(['sort_order'=>'ASC']);
        $modelListings = $this->Models->find('all');
...
//    }
    ...
    public function scenesphotoGallery($id = null) {
        ...
        if ($wat_pic->is_active == 1) {
            $w_mk = HTTP_ROOT . WATERMARK . 'result.png';
        }
        ...
        if ($this->request->is('post')) {
            ...
            for ($xi = 0; $xi < $nofile; $xi++) {
                ...
                if (isset($data['photo_filess'][$xi]['tmp_name'])) {
                    ...
                    $extname = explode(".", $data['photo_filess'][$xi]['name']);
                    $videoFiles = $customname = $this->Site_Nm . "-" . $data['scenes_id'] . '-' . ++$nom->id . '.' . end($extname);
                    ...
                    if ($extname == 'jpg' || $extname == 'JPG' || $extname == 'jpeg' || $extname == 'JPEG') {
                        imagejpeg($source, $thumbnail, 85);
                    } else if ($extname == 'png' || $extname == 'PNG') {
                        imagepng($source, $thumbnail, 85);
                    } else {
                        imagejpeg($source, $thumbnail, 85);
                    }
        ...
        $options_gallery = $this->ScenesPhotoGallery->find('list', ['keyField' => 'name', 'valueField' => 'name'])->where(['scenes_id' => $id]);
        $options_gallery_count = $this->ScenesPhotoGallery->find('list', ['keyField' => 'name', 'valueField' => 'name'])->where(['scenes_id' => $id])->count();
        $photo_gallery = $this->ScenesPhotoGallery->find('all')->where(['scenes_id' => $id])->order(['ScenesPhotoGallery.sort_order' => 'ASC']);
    ...
    public function modelGalleryPic() {
        if ($this->request->is('post')) {
            ...
            if (isset($data['myfile']['tmp_name'])) {
                ...
                $extname = explode(".", $data['myfile']['name']);
                $videoFiles = $customname = $this->Site_Nm . md5(time() . rand()) . '.' . end($extname);
                if (move_uploaded_file($data['myfile']['tmp_name'], $path . "main_" . $customname)) {
                    $this->Custom->GenerateThumbnail($path . "main_" . $customname, $path . 'thumb_' . $customname, 200, 200, 0.85);
                    $this->Custom->GenerateThumbnail($path . "main_" . $customname, $path . 'large_' . $customname, 900, 900, 0.85);
                    $fnm .= $videoFiles . ',';
                    $data['files_name'] = $videoFiles;
        ...
        if ($this->request->is('post')) {
            ...
            for ($xi = 0; $xi < $nofile; $xi++) {
                ...
                if (isset($data['photo_filess'][$xi]['tmp_name'])) {
                    ...
                    $path = $path . $data['gal_name'] . '/';
                    $extname = explode(".", $data['photo_filess'][$xi]['name']);
                    $videoFiles = $customname = $this->Site_Nm . "-" . str_replace(' ', '-', $editData->model_name) . '-' . ++$nom->id . '.' . end($extname);
...
//                }
    ...
    public function settingBanner() {
        ...
        if ($this->request->is('post')) {
            ...
            //pj($datas);
            $ss = $this->Bannersettings->updateAll([ 'slidemove' => $datas['slidemove'],  'speed' => $datas['speed'], 'autoplay' => $datas['autoplay'], 'loopplay' => $datas['loopplay']], ['id' => 1]);
    ...
    public function skincolor() {
        ...
        if ($this->request->is('post')) {
            ...
            if (isset($data['border_color'])) {
                $this->Skincolor->updateAll(['border_color' => $data['border_color']], ['id' => 1]);
            }
            if (isset($data['background_hover_color'])) {
                $this->Skincolor->updateAll(['background_hover_color' => $data['background_hover_color']], ['id' => 1]);
            }
            if (isset($data['nav_color'])) {
                $this->Skincolor->updateAll(['nav_color' => $data['nav_color']], ['id' => 1]);
            }
            if (isset($data['footer_top'])) {
                $this->Skincolor->updateAll(['footer_top' => $data['footer_top']], ['id' => 1]);
            }
            if (isset($data['content_middle'])) {
                $this->Skincolor->updateAll(['content_middle' => $data['content_middle']], ['id' => 1]);
            }
            if (isset($data['content_main_middle'])) {
                $this->Skincolor->updateAll(['content_main_middle' => $data['content_main_middle']], ['id' => 1]);
            }
            if (isset($data['footer_top_bg'])) {
                $this->Skincolor->updateAll(['footer_top_bg' => $data['footer_top_bg']], ['id' => 1]);
            }
            if (isset($data['body_bg'])) {
                $this->Skincolor->updateAll(['body_bg' => $data['body_bg']], ['id' => 1]);
            }
            if (isset($data['footer_last'])) {
                $this->Skincolor->updateAll(['footer_last' => $data['footer_last']], ['id' => 1]);
            }
            if (isset($data['login'])) {
                $this->Skincolor->updateAll(['login' => $data['login']], ['id' => 1]);
            }
            if (isset($data['member'])) {
                $this->Skincolor->updateAll(['member' => $data['member']], ['id' => 1]);
            }
            if (isset($data['logout'])) {
                $this->Skincolor->updateAll(['logout' => $data['logout']], ['id' => 1]);
            }
            if (isset($data['card'])) {
                $this->Skincolor->updateAll(['card' => $data['card']], ['id' => 1]);
            }
...
//            $this->Skincolor->save($dataEntity);
    ...
    public function scenetrailerVideo($id = null, $video_id = null) {
        if (empty($id)) {
            ...
            $vidId = $this->ScenesVideos->find('all')->where(['scene_id' => $data->id, 'is_tailer' => "Is tailer"])->first();
            $this->redirect(HTTP_ROOT . 'appadmins/scenetrailer_video/' . $data->id . '/' . $vidId->id);
        }

        $editData = $this->ScenesVideos->find('all')->where(['scene_id' => $id, 'id' => $video_id])->first();
        ...
        if ($this->request->is('post')) {
            ...
            //$data['release_date'] = date('Y-m-d', strtotime($data['release_date']));
            if (in_array($data['video_quality'], explode(",", $editData->video_quality)) == false) {

                if ($data['video_quality'] == "360p") {
                    exec('ffmpeg -i http://www.maturegapers.com/' . VIDEOS . $id . '/' . $editData->video_file . ' -vf scale=640x360:flags=lanczos  ' . VIDEOS . $id . '/360p_' . $editData->video_file . '', $out, $status);
                }
                if ($data['video_quality'] == "480p") {
                    exec('ffmpeg -i http://www.maturegapers.com/' . VIDEOS . $id . '/' . $editData->video_file . ' -s 640x480 -c:a copy ' . VIDEOS . $id . '/480p_' . $editData->video_file . '', $out1, $status1);
                }
                if ($data['video_quality'] == "720p") {
                    exec('ffmpeg -i http://www.maturegapers.com/' . VIDEOS . $id . '/' . $editData->video_file . ' -s 1280x720 -c:a copy ' . VIDEOS . $id . '/720p_' . $editData->video_file . '', $out2, $status2);
                }
                if ($data['video_quality'] == "1080p") {
                    exec('ffmpeg -i http://www.maturegapers.com/' . VIDEOS . $id . '/' . $editData->video_file . ' -s hd1080 -c:v libx264 -c:a copy ' . VIDEOS . $id . '/1080p_' . $editData->video_file . '', $out3, $status3);
                }
                if ($data['video_quality'] == "4K") {
                    exec('ffmpeg -i http://www.maturegapers.com/' . VIDEOS . $id . '/' . $editData->video_file . ' -s 3840x2160 -c:a copy ' . VIDEOS . $id . '/4K_' . $editData->video_file . '', $out3, $status3);
                }

                $data['video_quality'] = $editData->video_quality . "," . $data['video_quality'];

                $quali = explode(',', $data['video_quality']);

                if (in_array("360p", $quali) && in_array("720p", $quali) && in_array("1080p", $quali) && in_array("4K", $quali)) {
                    @unlink(VIDEOS . $id . '/' . $editData->video_file);
                }
            } else {
                $data['video_quality'] = $editData->video_quality;
            }
            $data['id'] = $video_id;
    ...
    public function scenemainVideo($id = null, $video_id = null) {
        ...
        if (empty($id)) {
            $data = $this->Scenes->find('all')->first();
            $vidId = $this->ScenesVideos->find('all')->where(['scene_id' => $data->id, 'is_tailer' => "Is main"])->first();
            $this->redirect(HTTP_ROOT . 'appadmins/scenemain_video/' . $data->id . '/' . $vidId->id);
        }
        ...
        if ($this->request->is('post')) {
            ...
            if (in_array($data['video_quality'], explode(",", $editData->video_quality)) == false) {
                ...
                if ($data['video_quality'] == "360p") {
                    $sdf = exec('ffmpeg -i http://www.maturegapers.com/' . VIDEOS . $id . '/' . $editData->video_file . ' -vf scale=640x360:flags=lanczos  ' . VIDEOS . $id . '/360p_' . $editData->video_file . '', $out, $status);
                }
                if ($data['video_quality'] == "480p") {
                    exec('ffmpeg -i http://www.maturegapers.com/' . VIDEOS . $id . '/' . $editData->video_file . ' -s 640x480 -c:a copy ' . VIDEOS . $id . '/480p_' . $editData->video_file . '', $out1, $status1);
                }
    ...
    public function scenebonusVideo($id = null, $video_id = null) {
        ...
        if (empty($id)) {
            $data = $this->Scenes->find('all')->first();
            $vidId = $this->ScenesVideos->find('all')->where(['scene_id' => $data->id, 'is_tailer' => "Is bonus"])->first();
            $this->redirect(HTTP_ROOT . 'appadmins/scenebonus_video/' . $data->id . '/' . $vidId->id);
        }
    ...
    public function leftBanner($id = null) {

        $datas = $this->CommericalBanners->find('all')->where(['CommericalBanners.type' => 1]);
        $options = $this->Models->find('list', ['keyField' => 'id', 'valueField' => 'model_name']);
        if (!empty($id)) {
            $editData = $this->CommericalBanners->find('all')->where(['CommericalBanners.type' => 1, 'id' => $id])->first();
        }
        if ($this->request->is('post')) {
            $data = $this->request->data;
        ...
        $this->set(compact('datas', 'options', 'id', 'editData'));
    }
            ...
            if (!empty($data['landing_photo'][0]['tmp_name'])) {
                for ($xi = 0; $xi < $nofile2; $xi++) {
                    ...
                    if (isset($data['landing_photo'][$xi]['tmp_name'])) {
                        ...
                        $extname = explode(".", $data['landing_photo'][$xi]['name']);
                        $videoFiles = $customname = md5(time() . rand()) . '.' . end($extname);
            ...
            if (!empty($data['landscroll_photo'][0]['tmp_name'])) {
                for ($xi = 0; $xi < $nofile3; $xi++) {
                    ...
                    if (isset($data['landscroll_photo'][$xi]['tmp_name'])) {
                        ...
                        $extname = explode(".", $data['landscroll_photo'][$xi]['name']);
                        $videoFiles = $customname = md5(time() . rand()) . '.' . end($extname);
    ...
    public function waterMarkSetting() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->WatermarkSettings->save($dataSave)) {
                $list = $this->WatermarkSettings->find('all')->where(['WatermarkSettings.id' => 1])->first();
                exec('convert ' . WATERMARK . $list->water_mark_image . ' -alpha set -background none -channel A -evaluate multiply ' . ($data['transparnt'] / 10) . ' +channel ' . WATERMARK . 'result.png', $out, $status);
    ...
    public function getFiles() {
        //echo getcwd();
        $dir = '/home/maturegapers/public_html/webroot/scenevideos/';

        $files1 = scandir($dir);

        $scenesname = $this->Scenes->find('all');

        foreach ($scenesname as $val) {
            if (!file_exists($dir . $val->scene_name . "-" . $val->id)) {
                //echo "File not there";
                @mkdir($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id, 777, true);
                chmod($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id, 0777);
                if (!file_exists($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-main')) {
                    mkdir($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-main', 777, true);
                    chmod($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-main', 0777);
                }
                if (!file_exists($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-bonus')) {
                    mkdir($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-bonus', 777, true);
                    chmod($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-bonus', 0777);
                }
                if (!file_exists($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-trailer')) {
                    mkdir($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-trailer', 777, true);
                    chmod($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-trailer', 0777);
                }
            }
        }
        ...
        if (empty($files1[2])) {
            foreach ($scenesname as $val) {
                if (!file_exists($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id)) {
                    //echo "File not there";
                    mkdir($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id, 777, true);
                    chmod($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id, 0777);
                    if (!file_exists($dir . $this->Custom->makeSeoUrl($val->scene_name) . "-" . $val->id . '/is-main')) {
        ...
        $this->set(compact('files1'));
    }

    public function updatefolder($filename = null) {
        $dir = '/home/maturegapers/public_html/webroot/scenevideos/' . $filename;
        $i = 0;
        $files1 = scandir($dir);
        foreach ($files1 as $val) {
            ...
            if ($val != '.' && $val != '..') {
                ...
                rename('scenevideos/' . $filename . '/' . $val, 'scenevideos/' . $filename . '/' . str_replace(" ", "-", $val));
                //echo $val;
                /*
                  exec('ffmpeg -i http://www.maturegapers.com/scenevideos/' . $filename . '/' . $val . ' -vf scale=640x360:flags=lanczos  scenevideos/' . $filename . '/360p_' . $val . '', $out, $status);

                  exec('ffmpeg -i http://www.maturegapers.com/scenevideos/' . $filename . '/' . $val . ' -s 640x480 -c:a copy scenevideos/' . $filename . '/480p_' . $val . '', $out1, $status1);

                  exec('ffmpeg -i http://www.maturegapers.com/scenevideos/' . $filename . '/' . $val . ' -s 1280x720 -c:a copy scenevideos/' . $filename . '/720p_' . $val . '', $out2, $status2);

                  exec('ffmpeg -i http://www.maturegapers.com/scenevideos/' . $filename . '/' . $val . ' -s hd1080 -c:v libx264 -c:a copy scenevideos/' . $filename . '/1080p_' . $val . '', $out3, $status3);
        ...
        $this->set(compact('files1', 'filename'));
    }

    public function updateftpvideo($filename = null, $subfolder = null, $trd = null) {
        $dir = '/home/maturegapers/public_html/webroot/scenevideos/' . $filename . '/' . $subfolder;
        $i = 0;
        $files1 = scandir($dir);
        foreach ($files1 as $val) {
            $i++;
            if ($val != '.' && $val != '..') {
                $is_tailer = "";
                if ($subfolder == 'is-main') {
                    $is_tailer = "Is main";
                }
                if ($subfolder == 'is-trailer') {
                    $is_tailer = "Is tailer";
                }
                if ($subfolder == 'is-bonus') {
                    $is_tailer = "Is bonus";
                }
                $scene_id = explode("-", $filename);

                if (!file_exists("/home/maturegapers/public_html/webroot/files/videos/" . end($scene_id))) {
                    mkdir("/home/maturegapers/public_html/webroot/files/videos/" . end($scene_id), 777, true);
                    chmod("/home/maturegapers/public_html/webroot/files/videos/" . end($scene_id), 0777);
                }


                // rename('scenevideos/' . $filename . '/' . $subfolder . '/' . $val, 'scenevideos/' . $filename . '/' . $subfolder . '/' . str_replace(" ", "-", $val));

                $vidchk = $this->ScenesVideos->find('all')->where(['video_name' => $val, 'is_tailer' => $is_tailer, 'scene_id' => end($scene_id)])->count();

                $getID3 = new \getID3();
                $fileinfo = $getID3->analyze('scenevideos/' . $filename . '/' . $subfolder . '/' . $val);

                //$width = $fileinfo['video']['resolution_x'];
                //$height = $fileinfo['video']['resolution_y'];                    
                $video_durations = $fileinfo['playtime_string'];

                $tim = time();
                if ($vidchk == 0) {
                    $vidInd = $this->ScenesVideos->newEntity();
                    $vidInd->is_tailer = $is_tailer;
                    $vidInd->scene_id = end($scene_id);
                    $vidInd->video_name = $val;
                    $vidInd->video_durations = $video_durations;
                    $vidInd->video_file = str_replace(" ", "-", $is_tailer . $tim) . str_replace(" ", "-", $val);
                    copy('scenevideos/' . $filename . '/' . $subfolder . '/' . $val, 'files/videos/' . end($scene_id) . '/' . str_replace(" ", "-", $is_tailer . $tim) . str_replace(" ", "-", $val));
                    $this->ScenesVideos->save($vidInd);
                } else {
                    @unlink('scenevideos/' . $filename . '/' . $subfolder . '/' . $val);
                }
            }
        }
        if ($trd != 1) {
            $this->Flash->success(__('Video received successfully'));
            $this->redirect(HTTP_ROOT . 'appadmins/updateftpvideo/' . $filename . '/' . $subfolder . '/1');
        }

        $this->set(compact('files1', 'filename', 'subfolder'));
    }

    public function manageMembers($id = null) {
        $allmemb = $this->Dbtable->find('all');
        if ($this->request->is('post')) {
            $data = $this->request->data;


            $dataSave = $this->Dbtable->newEntity();
            $data['password'] = sha1($data['password']);
            $dataSave = $this->Dbtable->patchEntity($dataSave, $data);
            $this->Dbtable->save($dataSave);
            $this->Flash->success(__('Member Added Successfully'));
            $this->redirect(HTTP_ROOT . 'appadmins/manage_members');
        }
        $this->set(compact('allmemb'));
    }

    public function deleteSourceFile($id = null) {
        $this->ScenesVideos->belongsTo('sv', ['className' => 'Scenes', 'foreignKey' => 'scene_id']);
        $allVideo = $this->ScenesVideos->find('all')->order(['scene_id' => 'ASC'])->contain(['sv']);
        //pj($allVideo);
        if (!empty($id)) {
            $data = $this->ScenesVideos->find('all')->where(['id' => $id])->first();
            @unlink('files/videos/' . $data->scene_id . '/' . $data->video_file);
            $this->Flash->success(__('Source file has been deleted Successfully'));
            $this->redirect(HTTP_ROOT . 'appadmins/delete_source_file');
        }
        $this->set(compact('allVideo'));
    }

    public function sceneSetting() {
        //SceneSettings
        $editdata = $this->SceneSettings->find('all')->first();
        $allscene = $this->Scenes->find('list', ['keyField' => 'id', 'valueField' => 'scene_name'])->where(['is_active'=>1]);
        if ($this->request->is('post')) {
            $data = $this->request->data;
            $dataSave = $this->SceneSettings->newEntity();
            $data['id']=$editdata->id;
            if ($data['type'] != 'Select manually') {
                $data['scene_id'] = 0;
            }
            $dataSave = $this->SceneSettings->patchEntity($dataSave, $data);
            $this->SceneSettings->save($dataSave);
            $this->Flash->success(__('Scene Settings Updated Successfully'));
            $this->redirect(HTTP_ROOT . 'appadmins/scene_setting');
        }
        $this->set(compact('allscene', 'editdata'));
    }
