...
//            echo json_encode(['id'=>$_COOKIE['scene_id']]);  
    ...
    public function sceneGallery($id = null, $gallery_name = null) {
        ...
        $this->set(compact('sceneallphotos', 'gallery_name', 'id', 'scenedetails'));
    }
    ...
    public function scenceSingle() {
        ...
        $scenevideo = $this->ScenesVideos->find('all')->where(['scene_id' => $id, 'is_tailer' => 'Is tailer'])->order(['sort_order' => 'ASC'])->first();

        $main_video_duration = $this->ScenesVideos->find('all')->where(['scene_id' => $id, 'is_tailer' => 'Is main'])->order(['sort_order' => 'ASC'])->first();
        ...
        $this->set(compact('scenesdetails', 'scenevideo', 'scenegallery', 'allscenes', 'main_video_duration', 'video_download'));
    }
...
// Zip archive will be created only after closing object
    ...
    public function videodownload($id, $format) {

        $file = $this->ScenesVideos->get($id);
        $path = HTTP_ROOT . VIDEOS . $file->scene_id . '/' . $format . $file->video_file;


        $this->response->body(function() use($path) {
            return file_get_contents($path);
        });
        return $this->response->withDownload($format . $file->video_name);
    }
